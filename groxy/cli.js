...
// or (node + 9router + cli.js). This repo path contains "9router", so both the
// groxy launcher and its server would be killed whenever 9router restarts.
// Rename this process (argv[0] rewrite → ps shows the new title) and preload a
// shield into the spawned server so its "next-server (vX)" title never lands.
process.title = "groxy";
const TITLE_SHIELD = path.join(__dirname, "hooks", "titleShield.js");

// Subcommands (`groxy xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.groxy/runtime
...
// Identifiers for killAllAppProcesses - only kill groxy specifically
const PROCESS_IDENTIFIERS = [
  'groxy'  // Only package name - avoid killing other apps
];
...
// Get app data dir (override via GROXY_DATA_DIR; default ~/.groxy)
function getAppDataDir() {
  if (process.env.GROXY_DATA_DIR) return process.env.GROXY_DATA_DIR;
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "groxy")
    : path.join(os.homedir(), ".groxy");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: groxy launcher (ps title "groxy" after process.title rename, or
// the raw node …/groxy/cli.js line before rename) or groxy's titled server
// (titleShield renames it to "groxy-server"). Never match bare "next-server" —
// that would kill the co-installed 9router server.
            const cmd = line.toLowerCase();
            const isAppProcess =
              /(^|\s)groxy(\s|$)/.test(cmd)
              || cmd.includes("groxy-server");
            if (isAppProcess) {
              const match = line.match(/^"(\d+)"/);
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: groxy launcher (ps title "groxy" after process.title
            // rename) or groxy's titled server (titleShield → "groxy-server").
            // Never match bare "next-server" — that would kill 9router.
            const cmd = line.toLowerCase();
            const isAppProcess =
              /(^|\s)groxy(\s|$)/.test(cmd)
              || cmd.includes("groxy-server");
...
// Find standalone server. Groxy lives in <repo>/groxy/, the 9router Next.js
// standalone build lives at <repo>/cli/app/ — reuse it instead of rebuilding.
// Override via GROXY_STANDALONE_DIR.
const repoRoot = path.resolve(__dirname, "..");
const standaloneDir = process.env.GROXY_STANDALONE_DIR
  || path.join(repoRoot, "cli", "app");
...
if (!fs.existsSync(serverPath)) {
  ...
  console.error("Build it with `npm run build` from the repo root, or set GROXY_STANDALONE_DIR.");
  process.exit(1);
}
...
function startServer(updatePromise) {
  ...
  function spawnServer() {
    ...
    crashLog = [];
    const child = spawn(RUNTIME, ["--dns-result-order=ipv4first", "--max-old-space-size=6144", "--require", TITLE_SHIELD, serverPath], {
      cwd: standaloneDir,
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 Groxy is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 Groxy is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
